...
static inline double calculate( double x, double y, double cx, double cy, double exponent, int maxIterations )
{
    ...
    double radius_z;
    double angle_z;
    double radius_lambda=pow(pow(lambda_x,2)+pow(lambda_y,2),0.5);
    double angle_lambda=atan2(lambda_y,lambda_x);
        ...
        for ( int k = maxIterations; k > 0; k-- )
        {
            ...
            if ( radius_z >= BailoutRadius )
                return calculateResult( maxIterations, k, radius_z, exponent );

            angle_z=atan2(zy,zx);
            zx=pow(radius_z,power)*cos(angle_z*power)+t*(radius_lambda/pow(radius_z,power))*cos(angle_lambda-power*angle_z)+(1-t)*(radius_lambda/pow(radius_z,power))*cos(angle_lambda+power*angle_z);
            zy=pow(radius_z,power)*sin(angle_z*power)+t*(radius_lambda/pow(radius_z,power))*sin(angle_lambda-power*angle_z)+(1-t)*(radius_lambda/pow(radius_z,power))*sin(angle_lambda+power*angle_z);
        }
...
template<Variant VARIANT>
static inline double calculateM( double x, double y, double cx, double cy, double exponent, int maxIterations )
{
    x=0;
    y=0;//this should be the critical point
    double lambda_x=cx;
    double lambda_y=cy;
    //double lambda_x=0.11;
    //double lambda_y=0;
    double power = parameter002;
    double t=parameter001;
    ...
    //seed
    double zx = pow(radius_lambda,1/(2*power))*cos(angle_lambda/(2*power));
    //critical point
    double zy = pow(radius_lambda,1/(2*power))*sin(angle_lambda/(2*power));
...
public:
    ...
    double operator()( double zx, double zy, int maxIterations )
    {
        ...
        return calculateM<VARIANT>( zx, zy, zx, zy, m_exponent, maxIterations );
    }
};
